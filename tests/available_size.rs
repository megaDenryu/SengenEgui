...
use sengen_egui::{
    ノード, ボタン, 使える大きさから組む, 大きさを決めた領域, 子, 文字表示, 横並び, 無し, 画素の組,
    縦横の論理画素, 縦積み,
};
...
#[test]
fn 大きさを決めた領域の中央寄せは子を左右の中央へ置く() {
    let eguiの本体 = egui::Context::default();
    let 木を組む = || -> ノード<&'static str> {
        大きさを決めた領域(画素の組(300.0, 100.0), 子![ボタン("中", "押した")])
            .中央寄せ()
            .into()
    };
    let 領域の左上 = egui::pos2(8.0, 8.0);
    let 中央 = 領域の左上 + egui::vec2(150.0, 10.0);
    assert_eq!(
        common::左クリックする(&eguiの本体, 中央, &木を組む),
        vec!["押した"]
    );
    let 左端 = 領域の左上 + egui::vec2(4.0, 10.0);
    assert!(common::左クリックする(&eguiの本体, 左端, &木を組む).is_empty());
}
