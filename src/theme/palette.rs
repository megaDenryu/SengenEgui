...
const 入力欄の範囲選択の地へ寄せる割合: 割合 = 割合::範囲へ丸めて生成する(0.55);
...
impl 配色 {
    ...
    /// 選ばれた項目の文字の色と入力欄の範囲選択の地の色。選ばれた項目の文字の色は、指定があればそれを、無ければ
    /// 地の色と文字色のうち強調色とのコントラスト比が大きい方を使う。入力欄の範囲選択の地は、選択した文字が普段の
    /// 文字色のまま描かれるため、強調色を地の色の方へ寄せて文字色との差を広げる。
    pub(super) fn 強調色まわりの色(&self) -> 強調色まわりの色 {
        let 見分けやすい方 = || {
