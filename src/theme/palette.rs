...
use super::contrast::コントラスト比;
use super::mix::二色を混ぜる;
use super::selection_colors::選ばれた項目と範囲選択の色;
use super::明暗;
...
const 範囲選択の地へ寄せる割合: 割合 = 割合::範囲へ丸めて生成する(0.55);
...
pub(super) struct 配色 {
    ...
    pub(super) 強調色の上の文字色: Option<Color32>,
}
...
impl 配色 {
    ...
    /// 選ばれた項目の文字の色と範囲選択の地の色。選ばれた項目の文字の色は、指定があればそれを、無ければ
    /// 地の色と文字色のうち強調色とのコントラスト比が大きい方を使う。範囲選択の地は、選択した文字が普段の
    /// 文字色のまま描かれるため、強調色を地の色の方へ寄せて文字色との差を広げる。
    pub(super) fn 選ばれた項目と範囲選択の色(
        &self,
    ) -> 選ばれた項目と範囲選択の色 {
        let 見分けやすい方 = || {
            let (強調色, 地の色, 文字色) = (self.強調色?, self.地の色?, self.文字色?);
            let 地の色の方が見分けやすい =
                コントラスト比(強調色, 地の色) >= コントラスト比(強調色, 文字色);
            Some(if 地の色の方が見分けやすい {
                地の色
            } else {
                文字色
            })
        ...
        選ばれた項目と範囲選択の色 {
            選ばれた項目の文字色: self.強調色の上の文字色.or_else(見分けやすい方),
            範囲選択の地: 寄せた色(self.強調色, self.地の色, 範囲選択の地へ寄せる割合),
        }
    }
}
