//! 選ばれた項目と範囲選択の色。egui は選ばれた項目（選択ボタン・選択欄の一覧・タブ列の選ばれた項目）の文字を選択の縁の色で描き、
//! 文字の範囲選択の地を強調色で塗る。選択の縁の色は入力欄のフォーカスの枠線にも使われ、テーマは強調色と文字色を
//! 半々に混ぜて導くため明るく、明るい強調色の地の上の文字が読めない。選択の縁の色は変えずに、選ばれた項目の文字にだけ
//! 別の色を当て、範囲選択の地だけを読める色へ差し替える。
//! egui の見た目にはこの2つの色の項目が無いため、テーマを適用するときに egui の一時記憶へ置き、部品が描くときに読む。

use egui::Color32;

/// 選ばれた項目と範囲選択の色とは、テーマが決めた、選ばれた項目の文字の色と、文字の範囲選択の地の色の組のことである。
/// どちらも None なら egui の既定の描き方のまま描く。
#[derive(Clone, Copy, Default)]
pub(crate) struct 選ばれた項目と範囲選択の色 {
    pub(super) 選ばれた項目の文字色: Option<Color32>,
    pub(super) 範囲選択の地: Option<Color32>,
}
...
impl 選ばれた項目と範囲選択の色 {
    ...
    /// 文字を範囲選択できる部品（入力欄・数値入力の編集中の欄・選択できる文字表示）を、範囲選択の地の色を差し替えた
    /// 内側の領域で描く。差し替えは内側の領域だけに効き、後に並ぶ部品へ漏れない。
    pub(crate) fn 範囲選択の地を差し替えて描く(
        self,
        ui: &mut egui::Ui,
        部品: impl egui::Widget,
    ) -> egui::Response {
