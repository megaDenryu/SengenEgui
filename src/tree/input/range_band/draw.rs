...
//! 部品はドラッグだけを受ける。egui は押した位置で一番上の部品がドラッグだけを受けるとき、その下の押すことを受ける部品
//! （外側の押せる領域）へ押下を渡さないため、動かさずに押して放しても外側の押せる領域は応答を発しない（`tests/range_band_pressable.rs`）。
//! 押すことも受ける形にしないのは、egui がそのときポインタが少し動くまでドラッグを始めないため、数画素の細かい調整が
