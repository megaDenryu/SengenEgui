...
//! 全体の範囲の外の値のつまみと位置の印の頭は、帯の端に、外を指す三角で描く。端にちょうどある値の形（縦の棒と下向きの三角）と
//! 見分けられるようにし、範囲の外の値を端の値と取り違えさせないためである。

use super::hit::今の区間と位置;
use super::layout::区間の帯の配置;
use crate::measure::{帯の値, 画素, 論理画素};
use crate::style::スタイル;

const つまみの幅: 論理画素 = 画素(6.0);
const 外を指す三角の長さ: 論理画素 = 画素(6.0);
const 位置の印の頭の半幅: 論理画素 = 画素(5.0);
const 位置の印の線の太さ: 論理画素 = 画素(2.0);
const 角の丸み: 論理画素 = 画素(2.0);
...
impl 区間の帯の描き方 {
    ...
    pub(super) fn 描く(
        &self, 描き手: &egui::Painter, 配置: &区間の帯の配置, 値: 今の区間と位置
    ) {
        let 帯 = 配置.帯の段;
        描き手.rect_filled(帯, 角の丸み.eguiへ渡す値(), self.地の色);
        let 開始の横 = 配置.値の横の位置(値.開始).eguiへ渡す値();
        let 終了の横 = 配置.値の横の位置(値.終了).eguiへ渡す値();
        if 値.開始 <= 値.終了 {
            let 区間 = egui::Rect::from_x_y_ranges(開始の横..=終了の横, 帯.y_range());
            描き手.rect_filled(区間, 0.0, self.区間の色);
        }
        for つまみの値 in [値.開始, 値.終了] {
            self.つまみを描く(描き手, 配置, つまみの値);
        }
        if let Some(位置) = 値.位置 {
            self.位置の印を描く(描き手, 配置, 位置);
        }
    }

    fn つまみを描く(
        &self, 描き手: &egui::Painter, 配置: &区間の帯の配置, 値: 帯の値
    ) {
        let 帯 = 配置.帯の段;
        let 横 = 配置.値の横の位置(値).eguiへ渡す値();
        let 向き = 配置.全体.位置関係(値);
        if let Some(三角) =
            区間の帯の配置::外を指す三角(横, 向き, 帯.y_range(), 外を指す三角の長さ)
        {
        ...
        描き手.rect_filled(つまみ, 角の丸み.eguiへ渡す値(), self.つまみの色);
    }
