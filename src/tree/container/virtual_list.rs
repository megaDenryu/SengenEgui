...
//! 見えている行の範囲を知りたい利用する側には、描画のたびに1回だけ範囲を応答として発する（`.見えている行を発する()`）。
//! 行を組む閉包の呼び出しから範囲を推し量らせないのは、画面が状態を書き換える経路を作らせないためである。
...
type 見えている行から応答を作る手続き<M> = Box<dyn Fn(見えている行の範囲) -> M>;
...
pub struct 仮想列型<M> {
    ...
    末尾追従指定: bool,
    最大高さ指定: Option<論理画素>,
    見えている行から応答を作る: Option<見えている行から応答を作る手続き<M>>,
}
...
impl<M> 仮想列型<M> {
    ...
    /// 描画のたびに、組んだ行の範囲から作った応答を1回だけ発する。行が1つも見えていないフレームでは発しない。
    pub fn 見えている行を発する(
        mut self,
        範囲から応答を作る: impl Fn(見えている行の範囲) -> M + 'static,
    ) -> Self {
        self.見えている行から応答を作る = Some(Box::new(範囲から応答を作る));
        self
    }
}
...
impl<M: Clone> 仮想列型<M> {
    ...
    fn 見えている行を積む(
        &self, 組んだ行: Range<usize>, 発行した応答: &mut Vec<M>
    ) {
        let Some(作る) = &self.見えている行から応答を作る else {
            return;
        };
        if let Some(範囲) = 見えている行の範囲::組む行の区間から作る(組んだ行)
        {
            発行した応答.push(作る(範囲));
        }
    }
...
impl<M: 'static> 仮想列型<M> {
    pub(crate) fn 写す<N: 'static>(
        ...
        let 見えている行から応答を作る = self.見えている行から応答を作る.map(|元の作る| {
            見えている行の応答を写す(元の作る, Rc::clone(&応答を変換する))
        });
...
fn 見えている行の応答を写す<M: 'static, N: 'static>(
    元の作る: 見えている行から応答を作る手続き<M>,
    応答を変換する: Rc<dyn Fn(M) -> N>,
) -> 見えている行から応答を作る手続き<N> {
    Box::new(move |範囲| 応答を変換する(元の作る(範囲)))
}
