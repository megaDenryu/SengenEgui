...
//!
//! 重ねる子を寄せ先へ描く手順は `overlay_place` が持つ。重ねる子は、初めて出す回には大きさだけを測って
//! 見えず、押せない。見えて押せるのは次の回からである（描き直しを求めるため、人の目には1フレームである）。
//! 測った大きさは利用する側が渡す識別子を鍵に記憶する。兄弟の部品が出入りしても鍵は変わらない。
...
impl<M> 重ね型<M> {
    ...
    /// 下地の全体を覆いで塗り、その真ん中へ子を重ねる。覆っている間、下地は押せずドラッグもできない。
