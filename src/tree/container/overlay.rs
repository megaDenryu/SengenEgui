...
//!
//! 重ねる子を寄せ先へ描く手順は `overlay_place` が持つ。重ねる子は、識別子ごとに最初に出した1回だけ
...
impl<M: Clone> 重ね型<M> {
    pub(crate) fn 描画する(
        ...
        let 下地の反応 = ui
            ...
            .inner;
        let mut 数えた区分一覧 = Vec::with_capacity(self.重ねる子一覧.len());
        for (置き方, 子) in &self.重ねる子一覧 {
            let 区分 = 置き方.記憶の区分();
            let 同じ区分の番号 = 数えた区分一覧
                .iter()
                .filter(|数えた| **数えた == 区分)
                .count();
            数えた区分一覧.push(区分);
            let 鍵 = egui::Id::new(("重ねる", &self.識別子, 区分, 同じ区分の番号));
