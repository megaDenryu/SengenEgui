...
impl 割合 {
    ...
    /// 倍精度の小数（f64）の 0 以上 1 以下の値から割合を作る。単精度（f32）の最も近い値へ丸めてから範囲を確かめるため、
    /// 1 をわずかに超える値は 1 になって受け取られることがある。範囲外なら丸めた後の値で理由を返す。
    pub fn 倍精度の小数から生成する(値: f64) -> Result<Self, 割合の範囲外> {
        Self::生成する(super::単精度の小数へ丸める(値))
    }

    /// 倍精度の小数（f64）の範囲外の値を 0 と 1 へ丸めて割合を作る。NaN は 0 にする。
    pub fn 倍精度の小数から範囲へ丸めて生成する(値: f64) -> Self {
        Self::範囲へ丸めて生成する(super::単精度の小数へ丸める(値))
    }
