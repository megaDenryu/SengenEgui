...
impl 論理画素 {
    ...
    /// 倍精度の小数（f64）から論理画素を作る。単精度（f32）の最も近い値へ丸める。
    /// 利用する側のドメインが f64 で計算した長さを画面へ渡すときに使う。
    pub fn 倍精度の小数から生成する(値: f64) -> Self {
        Self(super::単精度の小数へ丸める(値))
    }

    /// egui の API へ渡すための生の値。この口以外で f32 へ戻さない。寸法の計算には `倍精度の小数にする` を使う。
    pub const fn eguiへ渡す値(self) -> f32 {
        self.0
    }

    /// 利用する側のドメインが f64 で寸法を計算するための値。単精度の値をそのまま倍精度へ広げるため、誤差は生じない。
    /// 計算の結果を画面へ戻すときは `倍精度の小数から生成する` を使う。
    pub fn 倍精度の小数にする(self) -> f64 {
        f64::from(self.0)
    }
